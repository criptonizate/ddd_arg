...
export default function CalculadoraPublica({ config }: { config: CalculadoraConfig }) {
  ...
  const valid = g > 0 && h > 0
  const resultado = valid ? calcular(g, h, config) : null

  // Loguear una vez cuando ambos campos tienen valor (debounce 1.5s)
  useEffect(() => {
    if (!valid) { setLogged(false); return }
    if (logged) return
    const timer = setTimeout(() => {
      ...
      startLog(async () => {
        await logCalculadoraPublica({
          gramos: g, horas: h,
          costo_base: res.costoBase, precio_x3: res.x3, precio_x4: res.x4,
        })
      })
    }, 1500)
    return () => clearTimeout(timer)
  }, [g, h, valid, logged, config])
    ...
    <div className="w-full max-w-lg mx-auto">
      <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1.5">Filamento utilizado</label>
            <div className="relative">
              <input
                type="number" min="1" placeholder="ej. 150"
                value={gramos}
                onChange={(e) => { setGramos(e.target.value); setLogged(false) }}
                className="w-full border border-border rounded-xl px-4 py-3 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-ring pr-16"
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-muted-foreground font-medium">gramos</span>
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium mb-1.5">Tiempo de impresión</label>
            <div className="relative">
              <input
                type="number" min="0.1" step="0.5" placeholder="ej. 4"
                value={horas}
                onChange={(e) => { setHoras(e.target.value); setLogged(false) }}
              ...
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-muted-foreground font-medium">horas</span>
            </div>
          </div>
        </div>
        ...
        {resultado && (
          <div className="border-t border-border px-6 py-5 bg-secondary/20 space-y-3">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Precios de referencia</p>
            ...
            <p className="text-xs text-muted-foreground text-center pt-1">
              Estos son precios estimados. El precio final puede variar según el diseño y complejidad de la pieza.
            </p>
          </div>
        )}
      </div>
